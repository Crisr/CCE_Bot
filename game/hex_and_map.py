...
class CCE_Map:
    def __init__(self, data, scenario, bot_side):
        ...
        self.map["cells_coords"] = [
            [OffsetCoord(x, y) for x in range(self.map["col"])]
            for y in range(self.map["row"])
        ]
        self.map["cells_names"] = [
            [coords_to_literal(OffsetCoord(x, y)) for x in range(self.map["col"])]
            for y in range(self.map["row"])
        ]
        self.map["np_hexMap"] = np.array(
            self.map["cells_names"]
        )  # numpy 2D map of cell names (A1, A2, ...)
        ...
        self.hex_edges = [
            ...
            "E5",
            "E4",
        ]  # Edge 1 on top and then clockwise
